...
import { X, Moon, Sun, Monitor, Trash2, Globe, Book, Shield, Key } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
                <Key size={12} /> BYOK (Bring Your Own Key)
              </h3>
              <div className="flex items-center justify-between p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-100">
                <div className="flex flex-col">
                  <span className="text-sm font-medium text-slate-700">API 키 설정</span>
                  <span className="text-xs text-slate-500">OpenAI, Claude, Gemini 등 직접 사용</span>
                </div>
                <button
                  onClick={() => setShowBYOKModal(true)}
                  className="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg text-xs font-medium hover:from-blue-700 hover:to-indigo-700 transition-all shadow-sm hover:shadow-md"
                >
                  설정 열기
                </button>
              </div>
      ...
      {/* BYOK Modal */}
      <BYOKModal isOpen={showBYOKModal} onClose={() => setShowBYOKModal(false)} />
    </>
  );
};
